...
+ 第二步，根据您的需求，这一步分为两种情况:
  ...
  + 情况二，您需要付费咨询试用，因为想先咨询一些问题和了解我们的服务，以下为咨询流程：
    ...
    + 咨询流程：
        ...
        + 收到您的相关信息和材料（如成绩单和简历），并确认问题后，48小时（工作日）内，我们会将一份详细的，以事实为依据的答复报告发给您。
...
+ **Q：咨询试用中可以咨询哪些问题？**
    ...
    A：学生可以选择比较有高度的问题，比如以下是之前客户提出的一些问题：
...
如果学生的问题较为复杂，需要拆解成小问题。如果学生提问需要我们做大量的研究、信息搜索和数据分析后解答，需要在咨询试用之后购买[个性化咨询](https://zhuanlan.zhihu.com/p/139559712?)。

+ **为什么咨询是通过电子邮件进行的，可以直接电话或语音交流吗？**

  A：电子邮件作为咨询方式是经过我们过去经验的比对而作出的最优选择。暂时我们不提供电话或语音会议选项。学生/家长的咨询，一般会要求我们提供一定量的信息、数据和引用材料。而电子邮件的咨询方式有以下特点：
  ...
  + 问题不够具体或明确时，我们可以跟客户反向确认，以保证精准回答客户的问题。

+ **Q: 为什么要付费？**

    A: 因为
    
    + 学生使用咨询服务，获得了信息、指导等，我们付出了时间、精力和技能成本。因此学生需要支付咨询费用。
    ...
    如果会议后您决定继续购买申请服务（如选校、文书、咨询），该费用的50%将用来抵消后续服务的部分费用。
...
+ **Q：签署了咨询试用合同，可以取消订单吗？**
    ...
    + 如果学生/家长还未将成绩单、简历和最终问题列表发给我们，想取消订单，我们需要扣除已付费用的20%，剩余部分退还。
    + 如果学生/家长已经将成绩单、简历和最终问题列表发给我们，但我们还未提供答复报告，客户想取消订单，我们需要扣除已付费用的40%，剩余部分退还。
    + 如果我们已经提供了答复报告，客户想取消订单，因我们已完成服务，已付费用不予退还。
...
+ **Q: 每位客户可以有几次咨询试用机会？**
    ...
    A：每位学生只有一次试用咨询的机会。咨询试用后，如果学生还想继续咨询，可以购买我们的[个性化咨询服务](https://tessay.org/pages/consulting)。正常咨询的费用是1800元/封邮件，包含6个问答。每个问题我们的研究、调查和回答时间不得超过1小时。如果问题较复杂，需拆解成小问题。
